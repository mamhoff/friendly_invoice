require "rails_helper"

describe "Invoices:" do
  before do
    Rails.application.load_seed

    @unpaid_invoice = FactoryBot.create(:invoice, :pending, draft: false)
    @paid_invoice = FactoryBot.create(:invoice, :paid, draft: false)
    @draft_invoice = FactoryBot.create(:invoice, draft: true)
  end

  specify "User can see a list of invoices, most recent first", :js, :logged_in do
    visit invoices_path

    within "#js-list-form" do
      expect(find(:xpath, ".//tbody/tr[1]")).to have_content "A-[draft]"
      expect(find(:xpath, ".//tbody/tr[2]")).to have_content "A-2"
      expect(find(:xpath, ".//tbody/tr[2]")).to have_content "paid"
      expect(find(:xpath, ".//tbody/tr[3]")).to have_content "A-1"
    end
  end

  specify "User gets redirected to edit page after clicking on an draft invoice", :js, :logged_in do
    visit invoices_path
    click_link(@draft_invoice.to_s)
    expect(page).to have_content("Series")
    expect(page.current_path).to eql edit_invoice_path(@draft_invoice)
  end

  specify "User gets redirected to preview page after clicking on a paid invoice", :logged_in do
    visit invoices_path
    expect(page).to have_link(@paid_invoice.to_s)
    click_link(@paid_invoice.to_s)
    expect(page.current_path).to eql invoice_path(@paid_invoice)
  end

  specify "User can mark invoices as paid from the invoices list", :js, :logged_in do
    visit invoices_path

    find_field("select_all").click
    click_on "Set Paid"

    expect(page).to have_content "Successfully set as paid 1 invoices."
  end
end

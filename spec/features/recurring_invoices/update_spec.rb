...
feature "Recurring Invoices:" do
  scenario "User can update a recurring invoice", :js do
  ...
  scenario "User can't update a recurring invoice with invalid data", :js do
    recurring_invoice = FactoryBot.create(:recurring_invoice)
  ...
  scenario "User can add a new item to an existing recurring invoice", :js do
    recurring_invoice = FactoryBot.create(:recurring_invoice)
    visit edit_recurring_invoice_path(recurring_invoice)
    click_on "Add Line"
